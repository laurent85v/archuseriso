...
iso_name=archuseriso-mate
iso_label=AUIM
iso_publisher=
iso_application="Archuseriso Mate Live/Rescue medium"
...
_cleanup_airootfs() {
    local _cleanlist=(  ${work_dir}/x86_64/airootfs/root/.cache/
                        ...
                        ${work_dir}/x86_64/airootfs/root/customize_airootfs-mate.sh
...
make_packages() {
    local _lang=
    if [[ -n "${lang}" ]]; then
        _lang=$(grep -Ehv '^#|^$' ${script_path}/lang/"${lang}"/packages{-extra,-mate}.x86_64)
    fi
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' ${script_path}/packages{,-extra,-mate}.x86_64) ${_lang}" install
}
...
make_setup_mkinitcpio() {
    ...
    if ! $(grep -Ehv '^#|^$' ${script_path}/packages{,-extra*,-mate*}.x86_64 | sed -n 's/[[:blank:]]/\n/gp' | grep -qs '^nvidia$'); then
...
make_customize_airootfs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-mate.sh' run
