...
make_packages() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' ${script_path}/packages{,-extra,-i3}.x86_64) ${_lang} ${AUI_ADDITIONALPKGS:-}" install
}
...
make_customize_airootfs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-i3.sh' run
