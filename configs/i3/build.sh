...
iso_label=AUII
iso_publisher=""
iso_application="Archuseriso i3 Live/Rescue medium"
iso_version=""
install_dir=arch
work_dir=work
out_dir=out
...
_cleanup_airootfs() {
    ...
    [[ -e "${work_dir}/x86_64/airootfs/root/customize_airootfs-i3.sh" ]] && _cleanlist+=("${work_dir}/x86_64/airootfs/root/customize_airootfs-i3.sh")
...
make_packages() {
    local _lang=
    if [[ -n "${lang}" ]]; then
        _lang=$(grep -Ehv '^#|^$' "${script_path}"/lang/"${lang}"/packages{-extra,-i3}.x86_64 | sed ':a;N;$!ba;s/\n/ /g')
    fi
    if [ -n "${verbose}" ]; then
        mkarchiso -v -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' "${script_path}"/packages{,-extra,-i3}.x86_64 | sed ':a;N;$!ba;s/\n/ /g') ${_lang} ${AUI_ADDITIONALPKGS:-}" install
    else
        mkarchiso -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' "${script_path}"/packages{,-extra,-i3}.x86_64 | sed ':a;N;$!ba;s/\n/ /g') ${_lang} ${AUI_ADDITIONALPKGS:-}" install
    fi
}
...
make_customize_airootfs() {
    ...
    if [[ -e "${work_dir}/x86_64/airootfs/root/customize_airootfs-i3.sh" ]]; then
        if [ -n "${verbose}" ]; then
            mkarchiso -v -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-i3.sh' run
        else
            mkarchiso -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-i3.sh' run
        fi
    fi
