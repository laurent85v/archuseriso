...
iso_name=archuseriso-deepin
iso_label=AUID
iso_publisher=
iso_application="Archuseriso Deepin Live/Rescue medium"
...
_cleanup_airootfs() {
    local _cleanlist=(  ${work_dir}/x86_64/airootfs/root/.cache/
                        ...
                        ${work_dir}/x86_64/airootfs/root/customize_airootfs-deepin.sh
...
make_basefs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "cryptsetup device-mapper dhcpcd e2fsprogs inetutils jfsutils linux linux-firmware logrotate lvm2 man-db man-pages mdadm nano netctl perl s-nail shadow sysfsutils texinfo usbutils vi xfsprogs" install
...
make_packages() {
    local _lang=
    if [[ -n "${lang}" ]]; then
        _lang=$(grep -Ehv '^#|^$' ${script_path}/lang/"${lang}"/packages{-extra,-deepin}.x86_64)
    fi
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' ${script_path}/packages{,-extra,-deepin}.x86_64) ${_lang}" install
}
...
# Packages setup (airootfs)
make_packages_setup() {

    # Set spinfinity theme when plymouth available
...
make_setup_mkinitcpio() {
    ...
    if ! $(grep -Ehv '^#|^$' ${script_path}/packages{,-extra*,-deepin*}.x86_64 | sed -n 's/[[:blank:]]/\n/gp' | grep -qs '^nvidia$'); then
...
make_customize_airootfs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-deepin.sh' run
