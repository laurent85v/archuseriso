...
_cleanup_airootfs() {
    ...
    [[ -e "${work_dir}/x86_64/airootfs/root/customize_airootfs-kde.sh" ]] && _cleanlist+=("${work_dir}/x86_64/airootfs/root/customize_airootfs-kde.sh")
...
make_packages() {
    local _lang=
    if [[ -n "${lang}" ]]; then
        _lang=$(grep -Ehv '^#|^$' "${script_path}"/lang/"${lang}"/packages{-extra,-kde}.x86_64 | sed ':a;N;$!ba;s/\n/ /g')
    fi
    if [ -n "${verbose}" ]; then
        mkarchiso -v -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' "${script_path}"/packages{,-extra,-kde}.x86_64 | sed ':a;N;$!ba;s/\n/ /g') ${_lang} ${AUI_ADDITIONALPKGS:-}" install
    else
        mkarchiso -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -Ehv '^#|^$' "${script_path}"/packages{,-extra,-kde}.x86_64 | sed ':a;N;$!ba;s/\n/ /g') ${_lang} ${AUI_ADDITIONALPKGS:-}" install
    fi
}
...
make_customize_airootfs() {
    ...
    if [[ -e "${work_dir}/x86_64/airootfs/root/customize_airootfs-kde.sh" ]]; then
        if [ -n "${verbose}" ]; then
            mkarchiso -v -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-kde.sh' run
        else
            mkarchiso -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs-kde.sh' run
        fi
    fi
